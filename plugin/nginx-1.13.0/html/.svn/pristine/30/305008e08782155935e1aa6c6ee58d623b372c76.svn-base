...
angular.module('app')
    .controller('updateController', ['$rootScope', '$scope', '$http', '$state',
         function ($rootScope, $scope, $http, $state) {
             ...
             $scope.record.sendToAdmin = "2";
             validate(null);
             $scope.title = $rootScope.title = title;
             $scope.loading = true;
             $scope.record.intervalMinute = 30;

             $scope.withDept = true; // 默认按组织选择
             ...
             $scope.selectRange = function (val) {
                $("#showNodes").html('');
                $("#showNodesInput").val('');
                if (val == 1) {
                    $scope.withGroup = false;
                    $scope.withDept = true;
                    $scope.record.groupList = '';
                } else {
                    $scope.withGroup = true;
                    $scope.withDept = false;
                    $scope.record.deptList = '';
                }
             }
             ...
             function onGroupCheck(e, treeId, treeNode) {
                 ...
                 if (nodes.length > 0) {
                     for (var i = 0; i < nodes.length; i++) {
                         ...
                         if (pNode != null) {
                             arrNodeNames.push(pNode.name + "-" + nodes[i].name);
                         } else {
                             arrNodeNames.push(nodes[i].name);
                         }
                     }
                 }
                 console.log(arrNodeNames)
                 $("#showNodes").html(arrNodeNames.join("，"));
                 $("#showNodesInput").val(arrNodeNames.join("，"));
                 $("#showNodesInput").blur();
                 $scope.record.groupList = arrNodeIds.join(",");
             }
