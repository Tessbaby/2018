 ...
 */
 app.register.controller('manageCtrl', function ($rootScope, $scope, $http, $window) {
    ...
    // 获取表格
    function getTable () {
        $http.post($rootScope.setPath(8062) + '/admin/query?backModuleId=' + $rootScope.backModuleId, {})
    ...
    $scope.showPromise = function () {
        $.ajax({
            ...
            method: 'post',
            data: 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' + $rootScope.default.appid + '&redirect_uri=http%3A%2F%2F' + $rootScope.default.yPath + '%2Fauth%2Fregister%2Fwx%2Fopenid&response_type=code&scope=snsapi_userinfo&state=' + $rootScope.backModuleId + '#wechat_redirect',
            //dataType: "text",
