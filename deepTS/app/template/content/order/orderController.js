 ...
 */
 app.register.controller('orderCtrl', function ($rootScope, $scope, $http) {
    ...
    $scope.search.searchFun = function () {
        ...
        if ($scope.search.date || $scope.search.brandid || $scope.search.ordercode) {$rootScope.table.pageInfo.number = 0;};
        $rootScope.table.pageInfo.number = $rootScope.table.pageInfo.number == 0 ? 0 : $rootScope.table.pageInfo.number - 1
        $scope.tableUrl = '/goods/activity/back/order/query?date=' + $scope.search.date + '&brandid=' + $scope.search.brandid + '&ordercode=' + $scope.search.ordercode + '&page=' + $rootScope.table.pageInfo.number;
